...
use super::{IdentityError, Result, cert_store::CertStore};
...
impl MeterSigner {
    ...
    pub fn verify_signature(
        ...
        let public_key_bytes = self.cert_store.get_certificate(meter_id)
            .ok_or_else(|| IdentityError::CertificateError("Certificate not found".into()))?;

        let verifying_key = VerifyingKey::from_sec1_bytes(&public_key_bytes)
            .map_err(|e| IdentityError::CertificateError(format!("Invalid public key in cert: {}", e)))?;
